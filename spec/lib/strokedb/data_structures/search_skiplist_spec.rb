...
SearchSkiplist.with_optimizations(OPTIMIZATIONS) do |lang|

  describe "Empty SearchSkiplist [#{lang}]" do
  
    before(:each) do
      @maxlevel    = 8
      @probability = 0.5
      @list = SearchSkiplist.new(nil, :maxlevel => @maxlevel, :probability => @probability)
    end
  ...
  describe "SearchSkiplist#first_key [#{lang}]" do
    before(:each) do
      @maxlevel    = 8
      @probability = 0.5
      @list = SearchSkiplist.new(nil, :maxlevel => @maxlevel, :probability => @probability)
  ...
  describe "SearchSkiplist#find_nearest [#{lang}]" do
    before(:each) do
      @maxlevel    = 8
      @probability = 0.5
      @list = SearchSkiplist.new(nil, :maxlevel => @maxlevel, :probability => @probability)
